...
def test_low_sigma_never_increments_budget(action: int) -> None:
    ...
    # sigma at threshold (low-sigma path, strict-greater rule)
...
def test_on_position_closed_buy_decrements_long_by_n() -> None:
    ...
    layer.screen(FakeActionResult(action=2), mu=0.0, sigma=5.0)  # long=2
...
def test_on_position_closed_sell_decrements_short_by_n() -> None:
    ...
    layer.screen(FakeActionResult(action=4), mu=0.0, sigma=5.0)  # short=2
...
def test_budget_release_unblocks_previously_blocked_action() -> None:
    ...
    layer.screen(FakeActionResult(action=2), mu=0.0, sigma=5.0)  # long=2, full

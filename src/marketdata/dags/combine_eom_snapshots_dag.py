"""DAG: Manually combine & deduplicate the latest EOM snapshots across lanes.

Manually triggered (schedule=None). Monthly counterpart of
``combine_eod_snapshots``: consolidates the latest end-of-month MN1 snapshots
from the two monthly lanes (``create_eom_snapshot_2012`` (2012-01-02 →
2019-12-01, complete) and ``create_eom_snapshot_2020`` (2020-01-01 → now,
owns the current frontier)) into a single deduplicated series and overwrites
``create_eom_snapshot_2020``'s latest snapshot partition
(``marketdata/eom-snapshot/.../year=/month=/{ts}.parquet``, TIME_WINDOW=43200).
...
spans reveal whether the eom lanes do too).
"""
...
_TARGET_DAG = "create_eom_snapshot_2020"
_SOURCE_LANES = [
    "create_eom_snapshot_2012",
    "create_eom_snapshot_2020",
]
...
with DAG(
    dag_id="combine_eom_snapshots",
    description="Manually combine & dedup latest EOM MN1 snapshots from the 2012+2020 lanes into create_eom_snapshot_2020's latest partition",
    schedule=None,
    catchup=False,
    ...
    resolve_frontiers = KubernetesPodOperator(
        ...
        name="combine-eom-resolve-frontiers",
        namespace="airflow",
    ...
    combine_snapshots = KubernetesPodOperator(
        ...
        env_vars=[
            ...
            k8s.V1EnvVar(name="INTERVALS", value="MN1"),
        ...
        container_resources=k8s.V1ResourceRequirements(
            # MN1 frames are tiny (~760 monthly bars full-history); 1Gi is ample.

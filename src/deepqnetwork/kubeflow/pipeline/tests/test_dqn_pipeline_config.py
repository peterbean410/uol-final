...
class TestToCliArgs:
    ...
    def test_dueling_flag_present_when_true(self) -> None:
        config = DQNPipelineConfig(dueling=True)
        args = config.to_cli_args()
        assert "--dueling" in args

    def test_dueling_flag_absent_when_false(self) -> None:
        config = DQNPipelineConfig(dueling=False)
        args = config.to_cli_args()
        assert "--dueling" not in args

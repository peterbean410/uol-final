# FluxCD: Register forex-dqn DAG Bundle in Airflow HelmRelease

## Overview

This document describes the change required in the **FluxCD repo** to register the
`forex-dqn` DAG bundle with the Airflow 3 DAG processor. This enables Airflow
to automatically discover and load the DQN DAGs from this Git repository.

**Validates: Requirement DQN-R23**, DQN DAG files synced via GitDagBundle entry in the
Airflow HelmRelease.
...
- `forex-forecaster` → `probabilisticforecaster/kubeflow/airflow` (forecaster DAGs)

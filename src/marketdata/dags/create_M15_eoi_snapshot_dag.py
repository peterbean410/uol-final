...
download task in the download_price_bars_hourly DAG. On Sat–Sun (when
no download fires) the wait is bypassed so the snapshot chain stays
unbroken, `create_snapshot` then re-emits the prior snapshot's
...
# download_price_bars_hourly runs Mon–Fri (cron "0 * * * 1-5").
...
with DAG(
    dag_id="create_eoh_snapshot",
    default_args=default_args,
    description="Create FX end-of-interval price snapshots (M1, M5, M15) using the marketdata Helm chart image",
    schedule="0 * * * *",

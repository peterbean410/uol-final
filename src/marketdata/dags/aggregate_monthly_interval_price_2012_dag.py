"""DAG: Aggregate one calendar month of D1 bars into MN1, 2012 backfill.

Mirror of ``aggregate_monthly_interval_price_2020`` rolled back to
2012-02-01 (first run covers January 2012) and capped at 2020-01-01
(last run covers December 2019) so it doesn't overlap with the 2020
sibling.

Runs once per calendar month on the 1st at 00:00 UTC. Waits for the
last daily D1 aggregate of the previous month
(``aggregate_daily_interval_price_2012``) to complete, then runs the
aggregate-interval-price script to produce an MN1 (calendar-month) bar
for the just-completed month.

W1 is handled by ``aggregate_weekly_interval_price_2012``; a monthly
...
with DAG(
    ...
    dag_id="aggregate_monthly_interval_price_2012",
    default_args=default_args,
    description=(
        "Aggregate one calendar month of D1 bars into an MN1 interval-price "
        "partition, Jan 2012 through Dec 2019 backfill"
    ),
    schedule="0 0 1 * *",

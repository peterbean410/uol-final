"""DAG: Download price bars (D1, W1) via Kubernetes, monthly schedule.
...
in parallel within a single monthly DAG run.

TIME_WINDOW_IN_MINUTES is computed via Jinja so that it correctly
reflects the variable length of each calendar month.
"""
...
with DAG(
    dag_id="download_price_bars_monthly",
    default_args=default_args,
    description="Download FX price bars (D1, W1) monthly using the marketdata Helm chart image",
    schedule="0 0 1 * *",

"""DAG: Create end-of-day (EOD) tick snapshots via Kubernetes, 2012 backfill.
...
# download_tick_data_2012 runs Mon–Fri (cron "0 * * * 1-5"). The sensor looks
...
def _last_tick_hourly_logical_date(_dt, **context):
    ...
    `download_tick_data_2012` fires hourly on weekdays with `logical_date`
...
with DAG(
    ...
    catchup=True,
    tags=["marketdata", "tickdata", "snapshot", "eod", "2012"],
) as dag:
    ...
    wait_for_last_tick_download = ExternalTaskSensor(
        ...
        external_dag_id="download_tick_data_2012",
